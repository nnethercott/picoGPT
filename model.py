...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.fc_1 = config.linear_cls(
            config.n_embd, 4 * config.n_embd, bias=config.bias
        )
        self.fc_2 = config.linear_cls(
...
class CausalSelfAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, attn_mask=None):
        ...
        # attention mask from hf-like arrays
        attn_mask = (
            attn_mask.unsqueeze(1)
            .repeat((1, v.shape[1], 1))
            .unsqueeze(-1)  # B x nhs x T x 1
        )
...
class PicoGPT(nn.Module):
    def __init__(self, config):
        ...
        self.lm_head = config.linear_cls(
    ...
    def _beam_search(
        ...
        beams = [{"cum_log_prob": 0.0, "ids": input_ids}]
        stopped_beams = []
        seen_tokens = set()
        ...
        for _ in range(max_new_tokens):
            ...
            for beam in beams:
                ...
                for lp, i in zip(log_ps, ids):
                    new_beams.append(
                        {
                            "cum_log_prob": beam["cum_log_prob"] + lp,
                            "ids": torch.cat((beam["ids"], i.view(1, 1)), dim=-1),
                        }
                    )
            ...
            # keep `num_beams`
            beams = sorted(beams, key=lambda x: x["cum_log_prob"], reverse=True)[
                :num_beams
            ]
        ...
        return [
            beam["ids"][:, size:].squeeze(0) for beam in beams[:num_return_sequences]
        ]
